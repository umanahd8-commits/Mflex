...
# ---------- CONFIG ----------
# Your bot token (as requested to include)                                                            BOT_TOKEN = "8478769265:AAFk0HRmbbNwulr1DEu7-QYojsQ4yBv3kaA"

# Two admin IDs (you provided these)
ADMIN_IDS = [7753547171, 8303629661]                                                                  
DB_PATH = "earning_bot.db"

JOIN_FEE = 2000                                                                                       REFERRAL_BONUS = 1000
VIP_UPGRADE_COST = 5000
VIP_REFERRAL_BONUS = 1300                                                                             MIN_WITHDRAW = 4000

# Lucky spin probabilities (sum to 1.0)
SPIN_OUTCOMES = [                                                                                         ("100", 0.50),
    ("200", 0.30),
    ("500", 0.05),                                                                                        ("TRY_AGAIN", 0.15),
]
...
# Links                                                                                               UPDATES_CHANNEL_URL = "https://t.me/moniflex1"
HELP_SUPPORT_URL = "https://t.me/MONIFLEXBOT1"
                                                                                                      bot = telebot.TeleBot(BOT_TOKEN)
                                                                                                      # ---------- DB HELPERS ----------
def init_db():                                                                                            conn = sqlite3.connect(DB_PATH)
    cur = conn.cursor()
    cur.execute("""CREATE TABLE IF NOT EXISTS users (                                                         user_id INTEGER PRIMARY KEY,
        username TEXT,
        first_name TEXT,                                                                                      balance INTEGER DEFAULT 0,
        is_registered INTEGER DEFAULT 0,
        joined_at INTEGER,
        referrer_id INTEGER,
        is_vip INTEGER DEFAULT 0,                                                                             vip_since INTEGER,
        spins_used INTEGER DEFAULT 0,
        spin_week_start INTEGER DEFAULT 0
    )""")                                                                                                 cur.execute("""CREATE TABLE IF NOT EXISTS deposits (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        user_id INTEGER,
        amount INTEGER,                                                                                       status TEXT, -- awaiting_amount, pending, approved, rejected, cancelled
    ...
    cur.execute("""CREATE TABLE IF NOT EXISTS withdrawals (
        ...
        account_details TEXT,                                                                                 admin_receipt_file_id TEXT,
        created_at INTEGER,
    ...
    cur.execute("""CREATE TABLE IF NOT EXISTS referrals (
        ...
        referred_id INTEGER,
        deposit_id INTEGER,                                                                                   bonus_amount INTEGER,                                                                                 created_at INTEGER                                                                                )""")                                                                                                 cur.execute("""CREATE TABLE IF NOT EXISTS pending_actions (                                               id INTEGER PRIMARY KEY AUTOINCREMENT,                                                                 user_id INTEGER UNIQUE,                                                                               action TEXT,  -- awaiting_deposit_amount, awaiting_withdraw_amount, awaiting_account_details          data TEXT,                                                                                            created_at INTEGER                                                                                )""")                                                                                                 conn.commit()
    conn.close()

    # Migrate existing withdrawals table if needed                                                        migrate_db()

def migrate_db():
    """Simple migration - fix withdrawals table schema"""                                                 conn = sqlite3.connect(DB_PATH)
    cur = conn.cursor()
                                                                                                          try:
        ...
        cur.execute("""CREATE TABLE withdrawals (                                                                 id INTEGER PRIMARY KEY AUTOINCREMENT,
            user_id INTEGER,
            amount INTEGER,                                                                                       status TEXT,
            account_details TEXT,
            admin_receipt_file_id TEXT,                                                                           created_at INTEGER,
            processed_at INTEGER
        )""")
...
def db_execute(query, params=(), fetchone=False, fetchall=False, commit=False):
    ...
    result = None
    if fetchone:
        result = cur.fetchone()                                                                           if fetchall:
        result = cur.fetchall()
    if commit:
        conn.commit()                                                                                     conn.close()                                                                                          return result                                                                                                                                                                                           def insert_deposit(user_id, receipt_file_id):                                                             conn = sqlite3.connect(DB_PATH)                                                                       cur = conn.cursor()                                                                                   cur.execute(                                                                                              "INSERT INTO deposits (user_id, amount, status, receipt_file_id, created_at) VALUES (?, ?, ?, ?, ?)",                                                                                                       (user_id, None, "awaiting_amount", receipt_file_id, now_ts())
    )
...
def clear_pending_action(user_id):                                                                        db_execute("DELETE FROM pending_actions WHERE user_id = ?", (user_id,), commit=True)

# ---------- UTILS ----------
def now_ts():                                                                                             return int(time.time())

def ensure_user(user):
    u = db_execute("SELECT user_id FROM users WHERE user_id = ?", (user.id,), fetchone=True)              if not u:
        db_execute(
            "INSERT INTO users (user_id, username, first_name, joined_at) VALUES (?, ?, ?, ?)",
            (user.id, user.username or "", user.first_name or "", now_ts()),                                      commit=True
        )

def get_user_row(user_id):                                                                                return db_execute("SELECT * FROM users WHERE user_id = ?", (user_id,), fetchone=True)

def user_is_admin(user_id):                                                                               return user_id in ADMIN_IDS                                                                                                                                                                             def send_to_all_admins(text=None, **kwargs):                                                              # Send message to all admins. kwargs passed to send_message/send_photo, etc.                          for aid in ADMIN_IDS:                                                                                     try:                                                                                                      if kwargs.get("photo"):                                                                                   bot.send_photo(aid, kwargs["photo"], caption=text, reply_markup=kwargs.get("reply_markup"))                                                                                                             elif kwargs.get("document"):
                bot.send_document(aid, kwargs["document"], caption=text, reply_markup=kwargs.get("reply_markup"))                                                                                                       else:
                bot.send_message(aid, text, parse_mode=kwargs.get("parse_mode"), reply_markup=kwargs.get("reply_markup"))
        except Exception:                                                                                         # ignore failures to reach some admins
            pass
                                                                                                      # ---------- MARKUPS ----------
def main_menu_markup_for(user_id):
    user = get_user_row(user_id)                                                                          markup = types.ReplyKeyboardMarkup(resize_keyboard=True)
    if user and user[4] == 1:
        markup.row("💰 My Balance", "👥 Refer & Earn")
        markup.row("💳 Deposit / Pay Fee", "🎰 Lucky Spin")                                                   markup.row("⭐ VIP Upgrade", "🚧 Tasks (Coming Soon)")
        markup.row("💵 Withdraw", "ℹ️ Help / Support")
    else:
        markup.row("💳 Deposit / Pay Fee")                                                                    markup.row("ℹ️ Help / Support")
    return markup

def deposit_approve_buttons(deposit_id, user_id):                                                         markup = types.InlineKeyboardMarkup()
...
def admin_panel_markup():
    ...
    markup.add(                                                                                               types.InlineKeyboardButton("👥 Members", callback_data="admin_members"),
...
def handle_start(m: types.Message):
    ensure_user(m.from_user)                                                                              args = m.text.split()
    if len(args) > 1:
    ...
    if user_row and user_row[4] == 0:
        bot.send_message(m.chat.id, welcome_txt, reply_markup=main_menu_markup_for(m.from_user.id))           bot.send_message(m.chat.id, "Join our updates or contact support:", reply_markup=inline)
    else:
...
def admin_panel(m):
    print(f"Admin panel command received from user {m.from_user.id}")                                     if not user_is_admin(m.from_user.id):
        bot.send_message(m.chat.id, "❌ Unauthorized. You are not an admin.")
        return                                                                                            print(f"Sending admin panel to user {m.from_user.id}")
...
def debug_admin(m):
    """Debug command to check admin status"""                                                             user_id = m.from_user.id
...
def admin_block(m):
    if not user_is_admin(m.from_user.id):                                                                     bot.send_message(m.chat.id, "❌ Unauthorized.")
        return
    parts = m.text.split()
    if len(parts) != 2:                                                                                       bot.send_message(m.chat.id, "Usage: /admin_block <user_id>")
        return
    try:
        uid = int(parts[1])                                                                                   db_execute("UPDATE users SET is_registered = 0 WHERE user_id = ?", (uid,), commit=True)
        bot.send_message(m.chat.id, f"✅ User {uid} has been blocked/unregistered.")
        try:                                                                                                      bot.send_message(uid, "❌ Your account has been blocked by admin. Contact support for details.")
        except:
            pass                                                                                          except:
        bot.send_message(m.chat.id, "❌ Invalid user ID.")
                                                                                                      # REGULAR MESSAGE HANDLERS
@bot.message_handler(regexp=r"^ℹ️ Help / Support$")
def help_support_button(m):                                                                               ensure_user(m.from_user)
    txt = (
        "📞 Help & Support\n\n"
        "Need assistance? Tap the button below to contact our support bot."
    )
    inline = types.InlineKeyboardMarkup()                                                                 inline.add(types.InlineKeyboardButton("💬 Contact Support", url=HELP_SUPPORT_URL))
    bot.send_message(m.chat.id, txt, reply_markup=inline)
                                                                                                      @bot.message_handler(regexp="^💰 My Balance$")
def my_balance(m):                                                                                        ensure_user(m.from_user)
    user = get_user_row(m.from_user.id)                                                                   if not user or user[4] == 0:
        bot.send_message(m.chat.id, "You must pay and have your deposit approved by admin before accessing account features. Use 'Deposit / Pay Fee'.", reply_markup=main_menu_markup_for(m.from_user.id))          return
    balance = user[3]
    vip = user[7]                                                                                         txt = f"💳 *Your Account*\n\nBalance: ₦{balance:,}\nVIP: {'Yes' if vip else 'No'}\nRegistered: Yes"
...
def refer_and_earn(m):
    ...
    if not user or user[4] == 0:                                                                              bot.send_message(m.chat.id, "You must complete registration (pay and be approved) to access referral features. Use 'Deposit / Pay Fee'.", reply_markup=main_menu_markup_for(m.from_user.id))                return
...
def vip_upgrade(m):                                                                                       ensure_user(m.from_user)
    user = get_user_row(m.from_user.id)
    ...
    markup = types.InlineKeyboardMarkup()                                                                 markup.add(types.InlineKeyboardButton(f"Buy VIP for ₦{VIP_UPGRADE_COST:,}", callback_data="buy_vip"))
...
def lucky_spin_menu(m):
    ...
    user = get_user_row(m.from_user.id)                                                                   if not user or user[4] == 0:
        ...
        return                                                                                            spins_allowed = 2 if user[7] == 1 else 1
    ...
    markup = types.InlineKeyboardMarkup()
    markup.add(types.InlineKeyboardButton("🎯 Spin", callback_data="spin_now"))                           markup.add(types.InlineKeyboardButton("➕ Buy extra spin (₦100)", callback_data="buy_spin"))
    bot.send_message(m.chat.id, txt, reply_markup=markup)

@bot.message_handler(regexp="^🚧 Tasks \\(Coming Soon\\)$")                                           def tasks_coming_soon(m):
    txt = (
        "🚧 Tasks — Coming Soon\n\n"
...
def withdraw_cmd(m):
    ensure_user(m.from_user)                                                                              user = get_user_row(m.from_user.id)
    if not user or user[4] == 0:
        bot.send_message(m.chat.id, "You must pay the registration fee and be approved by admin before requesting withdrawals.", reply_markup=main_menu_markup_for(m.from_user.id))                                 return
...
def cb_set_deposit_amount(call: types.CallbackQuery):
    try:
        parts = call.data.split(":")                                                                          cmd = parts[0]
        deposit_id = int(parts[1])
        choice = parts[2].strip()
    except Exception:
        bot.answer_callback_query(call.id, "Invalid action.")
        return                                                                                        
    row = db_execute("SELECT id, user_id, amount, status, receipt_file_id FROM deposits WHERE id = ?", (deposit_id,), fetchone=True)                                                                            if not row:
        bot.answer_callback_query(call.id, "Deposit not found.")                                              return
                                                                                                          user_id = row[1]
    ...
    if choice == "other":                                                                                     bot.answer_callback_query(call.id, "Please type the amount you paid (numbers only).")
        bot.send_message(user_id, "Please reply with the amount you paid (numbers only).")
        return
    else:                                                                                                     try:
            amount = int(choice.replace(",", "").replace(" ", ""))
        except:
            bot.answer_callback_query(call.id, "Invalid amount.")                                                 return
        finalize_deposit_amount(deposit_id, amount)
        clear_pending_action(user_id)
        bot.answer_callback_query(call.id, f"Amount ₦{amount:,} recorded and forwarded to admins.")           bot.send_message(user_id, f"✅ Amount ₦{amount:,} recorded. Your receipt has been sent to admins for verification.")
...
def cb_approve_deposit(call: types.CallbackQuery):                                                        if not user_is_admin(call.from_user.id):
        bot.answer_callback_query(call.id, "Unauthorized")
        return
    parts = call.data.split(":")                                                                          action = parts[0]
    deposit_id = int(parts[1])
    user_id = int(parts[2])                                                                               deposit = db_execute("SELECT id, user_id, amount, status FROM deposits WHERE id = ?", (deposit_id,), fetchone=True)
    if not deposit:                                                                                           bot.answer_callback_query(call.id, "Deposit not found.")
        return
    current_status = deposit[3]
    if action == "approve_deposit":                                                                           if current_status != "pending":
        ...
        try:                                                                                                      bot.edit_message_reply_markup(call.from_user.id, call.message.message_id, reply_markup=None)
        except:
            pass                                                                                          elif action == "reject_deposit":
        if current_status not in ("pending", "awaiting_amount"):
            bot.answer_callback_query(call.id, "Deposit is not in a rejectable state.")
            return                                                                                            db_execute("UPDATE deposits SET status = ? WHERE id = ?", ("rejected", deposit_id), commit=True)
        clear_pending_action(user_id)                                                                         bot.send_message(user_id, "❌ Your payment receipt was rejected by admin. Please check and upload a valid receipt.")
        bot.answer_callback_query(call.id, "Deposit rejected.")
        try:                                                                                                      bot.edit_message_reply_markup(call.from_user.id, call.message.message_id, reply_markup=None)
        except:                                                                                                   pass

@bot.callback_query_handler(func=lambda c: c.data == "buy_vip")                                       def cb_buy_vip(call: types.CallbackQuery):
    ...
    if not user or user[4] == 0:
        ...
        return
    balance = user[3]                                                                                     if balance < VIP_UPGRADE_COST:
        bot.answer_callback_query(call.id, "Insufficient balance to buy VIP.")                                bot.send_message(call.from_user.id, f"Your balance is ₦{balance:,}. You need ₦{VIP_UPGRADE_COST:,} to buy VIP.")                                                                                            return
    db_execute("UPDATE users SET balance = balance - ?, is_vip = 1, vip_since = ? WHERE user_id = ?",
               (VIP_UPGRADE_COST, now_ts(), call.from_user.id), commit=True)                              bot.answer_callback_query(call.id, "VIP purchased!")
...
def cb_spin(call: types.CallbackQuery):
    ensure_user(call.from_user)
    user = get_user_row(call.from_user.id)                                                                if not user or user[4] == 0:
    ...
    spins_used = user[9] or 0                                                                             spins_left = max(0, spins_allowed - spins_used)

    if call.data == "spin_now":
        if spins_left <= 0:                                                                                       bot.answer_callback_query(call.id, "No free spins left for this week. Buy extra spin to play more.")
            bot.send_message(call.from_user.id, "No free spins left this week. You can buy extra spins for ₦100 each.")                                                                                                 return
        ...
        outcome = "TRY_AGAIN"
        for name, prob in SPIN_OUTCOMES:                                                                          cumulative += prob
            if r <= cumulative:
                outcome = name                                                                                        break
        if outcome == "TRY_AGAIN":
            bot.send_message(call.from_user.id, "😕 Try Again — no win this time. Better luck next spin!")                                                                                                          else:
            amount = int(outcome)
            db_execute("UPDATE users SET balance = balance + ? WHERE user_id = ?", (amount, call.from_user.id), commit=True)                                                                                            bot.send_message(call.from_user.id, f"🎉 You won ₦{amount:,}! It has been added to your balance.")
...
def cb_withdraw_admin(call: types.CallbackQuery):                                                         print(f"Withdrawal callback received: {call.data} from user {call.from_user.id}")

    if not user_is_admin(call.from_user.id):
        bot.answer_callback_query(call.id, "❌ Unauthorized")                                                 return

    try:                                                                                                      parts = call.data.split(":")
        action = parts[0]
        withdraw_id = int(parts[1])
        user_id = int(parts[2])                                                                           except Exception as e:
        bot.answer_callback_query(call.id, f"❌ Error: {e}")
        return                                                                                        
    wd = db_execute("SELECT id, user_id, amount, status FROM withdrawals WHERE id = ?", (withdraw_id,), fetchone=True)                                                                                          if not wd:
    ...
    print(f"Processing {action} for withdrawal {withdraw_id}, status: {wd[3]}")
        ...
        if wd[3] != "pending":                                                                                    bot.answer_callback_query(call.id, "❌ Already processed.")
            return
        ...
        # Update the message to remove buttons                                                                try:
        ...
        except Exception as e:                                                                                    print(f"Could not update message: {e}")

    elif action == "upload_withdraw_receipt":
        if wd[3] != "pending":                                                                                    bot.answer_callback_query(call.id, "❌ Already processed.")
            return

        # Create pending action for admin to upload receipt                                                   create_pending_action(call.from_user.id, "admin_upload_withdraw_receipt", str(withdraw_id))
        ...
        bot.send_message(                                                                                         call.from_user.id,
            ...
            f"Please upload the payment receipt (photo or document) for this withdrawal.",                        parse_mode="Markdown"
        )

# Handle admin receipt upload for withdrawals - NOW APPROVES AUTOMATICALLY AFTER RECEIPT UPLOAD       @bot.message_handler(content_types=['photo', 'document'], func=lambda m: get_pending_action(m.from_user.id) and get_pending_action(m.from_user.id)[2] == "admin_upload_withdraw_receipt")
def handle_admin_withdraw_receipt(m):
    ...
    wd = db_execute("SELECT user_id, amount, status FROM withdrawals WHERE id = ?", (withdraw_id,), fetchone=True)                                                                                              if not wd:
        bot.send_message(m.chat.id, "❌ Withdrawal not found.")
        clear_pending_action(m.from_user.id)
        return                                                                                        
    user_id = wd[0]
    amount = wd[1]                                                                                        current_status = wd[2]

    if current_status != "pending":
        bot.send_message(m.chat.id, f"❌ Withdrawal already processed (status: {current_status}).")           clear_pending_action(m.from_user.id)
        return
                                                                                                          # Check user balance before processing
    user_row = get_user_row(user_id)
    if not user_row:                                                                                          bot.send_message(m.chat.id, "❌ User not found.")
        clear_pending_action(m.from_user.id)
        return
    ...
    if user_row[3] < amount:
        ...
        clear_pending_action(m.from_user.id)
        return                                                                                        
    # Update withdrawal with receipt, deduct balance, and mark as completed                               db_execute("UPDATE users SET balance = balance - ? WHERE user_id = ?", (amount, user_id), commit=True)                                                                                                      db_execute("UPDATE withdrawals SET status = 'completed', admin_receipt_file_id = ?, processed_at = ? WHERE id = ?",
               (file_id, now_ts(), withdraw_id), commit=True)                                         
    # Send receipt to user
    try:                                                                                                      if file_type == 'photo':
            bot.send_photo(
                user_id,
                file_id,
        ...
        bot.send_message(m.chat.id, f"✅ Receipt uploaded but could not notify user: {e}")
                                                                                                          # Notify admin
    bot.send_message(
        m.chat.id,                                                                                            f"✅ *Withdrawal Processed Successfully!*\n\n"
        f"Withdrawal ID: `{withdraw_id}`\n"
        f"User: `{user_id}`\n"                                                                                f"Amount: ₦{amount:,}\n"
        f"Status: Completed ✅",
        parse_mode="Markdown"
    ...
    try:
        # Find the original withdrawal notification message and update it                                     bot.edit_message_text(
            chat_id=m.chat.id,
...
def admin_callbacks(call: types.CallbackQuery):
    ...
    if cmd == "admin_members":
        ...
        # Split long messages
        if len(txt) > 4000:                                                                                       parts = [txt[i:i+4000] for i in range(0, len(txt), 4000)]
            for part in parts:
                bot.send_message(call.from_user.id, part)
        else:                                                                                                     bot.send_message(call.from_user.id, txt)
        ...
        if not rows:                                                                                              bot.send_message(call.from_user.id, "No deposits found.")
            bot.answer_callback_query(call.id, "No deposits")
            return                                                                                    
        txt = "📥 Deposits (latest 50)\n\n"
        for r in rows:                                                                                            created = datetime.datetime.fromtimestamp(r[4]).strftime("%Y-%m-%d %H:%M") if r[4] else "N/A"
            amt = r[2] if r[2] is not None else "(not set)"
            txt += f"ID:{r[0]} User:{r[1]} Amount:{amt} Status:{r[3]} At:{created}\n"

        if len(txt) > 4000:                                                                                       parts = [txt[i:i+4000] for i in range(0, len(txt), 4000)]
            for part in parts:
                bot.send_message(call.from_user.id, part)                                                     else:
            bot.send_message(call.from_user.id, txt)                                                          bot.answer_callback_query(call.id, "Deposits listed.")
                                                                                                          elif cmd == "admin_withdrawals":
        rows = db_execute("SELECT id, user_id, amount, status, account_details, created_at FROM withdrawals ORDER BY created_at DESC LIMIT 50", fetchall=True)                                                      if not rows:
            bot.send_message(call.from_user.id, "No withdrawals found.")
            bot.answer_callback_query(call.id, "No withdrawals")                                                  return
        ...
        if len(txt) > 4000:
            ...
            for part in parts:                                                                                        bot.send_message(call.from_user.id, part)
        else:
            bot.send_message(call.from_user.id, txt)
        bot.answer_callback_query(call.id, "Withdrawals listed.")                                     
    elif cmd == "admin_referrals":
        rows = db_execute("SELECT referrer_id, referred_id, bonus_amount, created_at FROM referrals ORDER BY created_at DESC LIMIT 100", fetchall=True)                                                             if not rows:
    ...
    if pending:
        ...
        data = pending[3]
            ...
            if m.text and m.text.strip().isdigit():
                try:
                    amount = int(m.text.strip())                                                                      except:
                    bot.send_message(m.chat.id, "Please send a numeric amount only.")
                    return
                deposit_id = int(data)                                                                                finalize_deposit_amount(deposit_id, amount)
                clear_pending_action(m.from_user.id)
                bot.send_message(m.chat.id, f"✅ Amount ₦{amount:,} recorded. Your receipt has been sent to admins for verification.")                                                                                      forward_deposit_to_admin(deposit_id)
                return
            else:                                                                                                     bot.send_message(m.chat.id, "Please send a numeric amount only.")
                return

        elif action == "awaiting_withdraw_amount":                                                                if m.text and m.text.strip().isdigit():
                try:
                    amount = int(m.text.strip())                                                                      except:
                    bot.send_message(m.chat.id, "Please send a numeric amount only.")
                    return                                                                            
                ...
                if not user or user[4] == 0:
                    ...
                    clear_pending_action(m.from_user.id)                                                                  return

                if amount < MIN_WITHDRAW:                                                                                 bot.send_message(m.chat.id, f"Minimum withdrawal is ₦{MIN_WITHDRAW:,}. Please enter an amount ≥ ₦{MIN_WITHDRAW:,}.")                                                                                        return
                                                                                                                      if user[3] < amount:
                    bot.send_message(m.chat.id, f"Insufficient balance. Your balance is ₦{user[3]:,}.")                                                                                                                         return

                create_pending_action(m.from_user.id, "awaiting_account_details", str(amount))                        bot.send_message(m.chat.id, "📝 *Please provide your account details*:\n\nYou can send:\n- Bank name:\n Account number:\n Account name:\n\n\nmake sure your account details are correct to aviod sending funds to wrong account")
                return
            else:                                                                                                     bot.send_message(m.chat.id, "Please send a numeric amount for withdrawal.")
                return

        elif action == "awaiting_account_details":                                                                try:
                amount = int(data)
            except:
                bot.send_message(m.chat.id, "Error processing withdrawal amount. Please start over.")                 clear_pending_action(m.from_user.id)
                return

            account_details = m.text.strip()                                                                      wid = insert_withdrawal(m.from_user.id, amount, account_details)
            clear_pending_action(m.from_user.id)
            ...
            return                                                                                    
    # Default fallback
    txt = (                                                                                                   "I didn't understand that. Use the menu below.\n\n"
...
# ---------- ADMIN PANEL (inline) ----------                                                          @bot.message_handler(commands=['adminpanel'])
def admin_panel(m):
    if not user_is_admin(m.from_user.id):
        bot.send_message(m.chat.id, "Unauthorized.")
        return
...
def admin_callbacks(call: types.CallbackQuery):
    ...
    if cmd == "admin_members":
        ...
        rows = db_execute("SELECT id, user_id, amount, status, created_at FROM deposits ORDER BY created_at DESC LIMIT 50", fetchall=True)                                                                          if not rows:
            bot.send_message(call.from_user.id, "No deposits found.")
            bot.answer_callback_query(call.id, "No deposits")                                                     return

        txt = "📥 Deposits (latest 50)\n\n"
        for r in rows:                                                                                            created = datetime.datetime.fromtimestamp(r[4]).strftime("%Y-%m-%d %H:%M") if r[4] else "N/A"
            amt = r[2] if r[2] is not None else "(not set)"                                                       txt += f"ID:{r[0]} User:{r[1]} Amount:{amt} Status:{r[3]} At:{created}\n"
        ...
        if not rows:                                                                                              bot.send_message(call.from_user.id, "No withdrawals found.")
            bot.answer_callback_query(call.id, "No withdrawals")                                                  return

        txt = "💸 Withdrawals (latest 50)\n\n"                                                                for r in rows:
            created = datetime.datetime.fromtimestamp(r[5]).strftime("%Y-%m-%d %H:%M") if r[5] else "N/A"                                                                                                               account_preview = r[4][:50] + "..." if r[4] and len(r[4]) > 50 else (r[4] or "No details")
            txt += f"ID:{r[0]} User:{r[1]} Amount:₦{r[2]:,} Status:{r[3]} Account:{account_preview} At:{created}\n"
    ...
    elif cmd == "admin_referrals":                                                                            rows = db_execute("SELECT referrer_id, referred_id, bonus_amount, created_at FROM referrals ORDER BY created_at DESC LIMIT 100", fetchall=True)
        if not rows:
            bot.send_message(call.from_user.id, "No referrals found.")                                            bot.answer_callback_query(call.id, "No referrals")
            return
        ...
        if len(txt) > 4000:
            ...
            bot.send_message(call.from_user.id, txt)                                                          bot.answer_callback_query(call.id, "Referrals listed.")

    elif cmd == "admin_add_balance_help":                                                                     bot.send_message(call.from_user.id, "To add balance use the command:\n/admin_add_balance <user_id> <amount>\nExample:\n/admin_add_balance 123456789 1000")
        bot.answer_callback_query(call.id, "How-to sent.")
                                                                                                          elif cmd == "admin_block_help":
        bot.send_message(call.from_user.id, "To block/unregister a user use the command:\n/admin_block <user_id>\nExample:\n/admin_block 123456789")
        bot.answer_callback_query(call.id, "How-to sent.")                                            
